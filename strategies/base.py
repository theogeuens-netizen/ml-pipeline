...
class Strategy(ABC):
    ...
    name: str = "base"
    version: str = "1.0.0"
    live: bool = False  # If True, executes with real money via LiveExecutor
    ...
    def check_time_exit(
        ...
        Args:
            ...
            max_hold_hours: Maximum hours to hold
        ...
        if hold_hours >= max_hold_hours:
            return Signal(
                ...
                side=Side.SELL,
                reason=f"Time exit: {hold_hours:.1f}h >= {max_hold_hours}h",
                market_id=position.market_id,
                price_at_signal=float(position.current_price or position.entry_price),
                edge=0,
                confidence=1.0,
    ...
    def check_profit_exit(
        ...
        Args:
            position: Current position
            profit_target_pct: Target profit percentage (e.g., 0.05 for 5%)

        Returns:
            Exit signal if profit target reached, None otherwise
        """
        ...
        if pnl_pct >= profit_target_pct:
            return Signal(
                token_id=position.token_id,
                side=Side.SELL,
                reason=f"Profit target: {pnl_pct:.1%} >= {profit_target_pct:.1%}",
                market_id=position.market_id,
                price_at_signal=float(position.current_price or position.entry_price),
                edge=0,
                confidence=1.0,
