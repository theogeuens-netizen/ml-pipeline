...
async def get_live_trading_summary(db: Session = Depends(get_db)):
    """
    Get live trading summary for the dashboard.

    Returns:
    - wallet_balance: Current USDC balance in Polymarket wallet
    - open_positions: Live open positions
    - realized_pnl: Sum of realized P&L from closed live positions
    - unrealized_pnl: Sum of unrealized P&L from open live positions
    - total_pnl: realized + unrealized
    - trade_count: Total live trades executed
    - strategies: List of live-enabled strategies with their stats
    """
    from src.executor.clients.order_client import get_order_client
    from strategies.loader import load_strategies
    ...
    try:
        ...
        open_live_positions = [p for p in live_positions if p.status == PositionStatus.OPEN.value]
        closed_live_positions = [p for p in live_positions if p.status == PositionStatus.CLOSED.value]
        ...
        realized_pnl = sum(
            ...
            for p in closed_live_positions
        )
        total_pnl = realized_pnl + unrealized_pnl
        ...
        # Get live trades count
        live_trades = db.query(ExecutorTrade).filter(
            ExecutorTrade.is_paper == False,
        ).count()

        # Get live-enabled strategies
        all_strategies = load_strategies()
        live_strategies = [s for s in all_strategies if getattr(s, 'live', False)]

        # Stats per live strategy
        strategy_stats = []
        for strategy in live_strategies:
            strategy_positions = [p for p in live_positions if p.strategy_name == strategy.name]
            strategy_open = [p for p in strategy_positions if p.status == PositionStatus.OPEN.value]
            strategy_closed = [p for p in strategy_positions if p.status == PositionStatus.CLOSED.value]

            strategy_realized = sum(float(p.realized_pnl) if p.realized_pnl else 0 for p in strategy_closed)
            strategy_unrealized = sum(float(p.unrealized_pnl) if p.unrealized_pnl else 0 for p in strategy_open)

            wins = len([p for p in strategy_closed if p.realized_pnl and float(p.realized_pnl) > 0])
            losses = len([p for p in strategy_closed if p.realized_pnl and float(p.realized_pnl) <= 0])

            strategy_stats.append({
                "name": strategy.name,
                "open_positions": len(strategy_open),
                "closed_positions": len(strategy_closed),
                "realized_pnl": round(strategy_realized, 2),
                "unrealized_pnl": round(strategy_unrealized, 2),
                "total_pnl": round(strategy_realized + strategy_unrealized, 2),
                "wins": wins,
                "losses": losses,
        ...
        for p in open_live_positions:
            ...
            formatted_positions.append({
                ...
                "token_side": "YES" if market and p.token_id == market.yes_token_id else "NO",
