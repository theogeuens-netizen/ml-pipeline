...
class PolymarketOrderClient:
    ...
    def get_fee_rate_bps(self, token_id: str) -> int:
        """
        Get the taker fee rate for a token in basis points.

        Most markets have 0% fees, but 15-minute crypto markets
        can have up to 10% (1000 bps) taker fees.

        Args:
            token_id: Polymarket token ID

        Returns:
            Fee rate in basis points (e.g., 1000 = 10%, 0 = no fee)
        """
        try:
            import requests
            ...
            proxies = None
            if self.proxy_enabled and hasattr(settings, 'trading_proxy_url') and settings.trading_proxy_url:
                proxies = {
                    'http': settings.trading_proxy_url,
                    'https': settings.trading_proxy_url,
                }
            ...
            fee_rate = data.get("base_fee", data.get("fee_rate_bps", 0))
            logger.debug(f"Fee rate for token {token_id[:20]}...: {fee_rate} bps")
            return fee_rate
        except Exception as e:
            logger.warning(f"Failed to get fee rate for {token_id}: {e}, defaulting to 0")
            return 0
    ...
    def place_limit_order(
        ...
        try:
            ...
            # Get the fee rate for this token (varies by market)
            fee_rate = self.get_fee_rate_bps(token_id)

            logger.info(
                f"Placing order: token={token_id}, side={side}, "
                f"price={rounded_price}, size_shares={size_shares}, size_usd={size_usd}, fee_rate={fee_rate}bps"
            )

            # Create and post order with dynamic fee rate
            ...
            order = self.client.create_and_post_order(
                OrderArgs(
                    ...
                    fee_rate_bps=fee_rate,  # Dynamically fetched fee rate
                )
            )
