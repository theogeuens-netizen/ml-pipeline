...
class PositionSizer:
    ...
    def calculate_size(
        ...
        # Check for per-strategy sizing in decision_inputs (takes priority)
        size_pct = None
        min_size_usd = None
        if hasattr(signal, 'decision_inputs') and signal.decision_inputs:
            size_pct = signal.decision_inputs.get('size_pct')
            min_size_usd = signal.decision_inputs.get('min_size_usd')

        if size_pct is not None and size_pct > 0:
            # Use per-strategy percentage sizing with optional minimum
            pct_size = kelly_capital * size_pct
            if min_size_usd is not None and min_size_usd > 0:
                # max(min_size_usd, capital * size_pct)
                size = max(min_size_usd, pct_size)
                logger.debug(
                    f"Using per-strategy sizing: max(${min_size_usd:.2f}, "
                    f"{size_pct:.1%} of ${kelly_capital:.2f}) = ${size:.2f}"
                )
            else:
                size = pct_size
                logger.debug(
        ...
        elif sizing.method == SizingMethod.PERCENT:
            size = self._percent_size(sizing)
    ...
    def _percent_size(self, sizing: SizingConfig) -> float:
        """
        Calculate position size as percentage of wallet balance.

        For live trading, fetches real balance from Polymarket.
        For paper trading, uses configured paper balance.
        """
        ...
        try:
            ...
            logger.info(f"Percent sizing: wallet balance = ${wallet_balance:.2f}")
        except Exception as e:
            # Fallback to paper balance if live not available
            logger.warning(f"Could not fetch live wallet balance ({e}), using paper balance")
            wallet_balance = 10000.0  # Default paper balance

        size = wallet_balance * sizing.percent_of_wallet
        logger.debug(
            f"Percent sizing: {sizing.percent_of_wallet:.1%} of ${wallet_balance:.2f} = ${size:.2f}"
        )
        return size
