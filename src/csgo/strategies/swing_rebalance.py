...
class CSGOSwingRebalanceStrategy(CSGOStrategy):
    ...
    def on_tick(self, tick: Tick) -> Optional[Action]:
        ...
        self._market_states[market_id] = {
            ...
            # Position tracking
            "yes_shares": self.position_size / yes_price,
            "no_shares": self.position_size / no_price,
            "yes_cost_basis": yes_price,
            "no_cost_basis": no_price,
            # Trade tracking
            "last_trade_time": now,
            "rebalance_count": 0,
    ...
    def _execute_pending_buy(
        ...
        new_shares = size_usd / buy_price

        if token_type == "YES":
            old_shares = state["yes_shares"]
            old_cost = state["yes_cost_basis"]
            total_shares = old_shares + new_shares
            if total_shares > 0:
                state["yes_cost_basis"] = (
                    (old_shares * old_cost + new_shares * buy_price) / total_shares
                )
            state["yes_shares"] = total_shares
        else:
            old_shares = state["no_shares"]
            old_cost = state["no_cost_basis"]
            total_shares = old_shares + new_shares
            if total_shares > 0:
                state["no_cost_basis"] = (
                    (old_shares * old_cost + new_shares * buy_price) / total_shares
                )
            state["no_shares"] = total_shares
