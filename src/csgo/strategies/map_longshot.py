...
class CSGOMapLongshotStrategy(CSGOStrategy):
    ...
    def on_tick(self, tick: Tick) -> Optional[Action]:
        ...
        # Check NO side opportunity (use actual NO price from order book)
    ...
    def on_position_update(self, position, tick: Tick) -> Optional[Action]:
        ...
        # Calculate current price for our token (use actual NO price from order book)
        if token_type == "YES":
    ...
    def _update_price_history(self, market_id: int, tick: Tick) -> None:
        """Add current prices (both YES and NO) to history."""
        ...
        # Store both YES and NO prices to support accurate drop detection on both sides
        # YES and NO have separate order books - prices don't sum to 100%
        yes_price = tick.yes_price
        no_price = tick.no_price or (1 - yes_price)  # Fallback to derived if not available
    ...
    def _calculate_drop(self, market_id: int, token_type: str) -> Optional[float]:
        ...
        # Get prices within window - history now has (timestamp, yes_price, no_price) tuples
        prices_in_window = [
            (ts, yes_p, no_p) for ts, yes_p, no_p in history if ts >= cutoff
        ]

        if len(prices_in_window) < 2:
            return None

        # Use actual prices for each token type (not derived)
        if token_type == "YES":
            # YES dropped = max YES price - current YES price
            max_price = max(yes_p for _, yes_p, _ in prices_in_window)
            current_price = prices_in_window[-1][1]  # yes_price
            drop = max_price - current_price
        else:
            # NO dropped = max NO price - current NO price (using actual NO prices)
            max_no_price = max(no_p for _, _, no_p in prices_in_window)
            current_no_price = prices_in_window[-1][2]  # no_price
            drop = max_no_price - current_no_price
