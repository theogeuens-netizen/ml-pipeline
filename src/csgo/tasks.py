...
def _update_match_from_clob(
    ...
    - /book: Full orderbook with bids and asks - use this to get actual spread
    - /midpoint: Current mid price (calculated from orderbook)

    We extract best bid/ask from the orderbook and calculate spread = ask - bid.

    IMPORTANT: We fetch BOTH YES and NO prices separately because Polymarket
    has separate order books for each token. Prices don't sum to 100% due to
    bid-ask spreads and market inefficiencies.
    ...
    try:
        # Get full orderbook to extract best bid/ask
        ...
        # Best bid = highest price someone is willing to pay (buy)
        # Polymarket orderbook is sorted ascending, so we need max()
        if bids:
            yes_bid = max(float(b.get("price", 0)) for b in bids)
            if yes_bid > 0:
                match.best_bid = Decimal(str(yes_bid))
                changed = True

        # Best ask = lowest price someone is willing to sell at
        # Polymarket orderbook asks are sorted descending, so we need min()
        if asks:
            yes_ask = min(float(a.get("price", 0)) for a in asks)
            if yes_ask > 0:
                match.best_ask = Decimal(str(yes_ask))
                changed = True

        # Calculate spread = ask - bid (the simple and correct way)
        if yes_bid and yes_ask and yes_bid > 0 and yes_ask > 0:
            spread = yes_ask - yes_bid
            match.spread = Decimal(str(spread))
            # Calculate mid price from bid/ask
            yes_mid = (yes_bid + yes_ask) / 2
            match.yes_price = Decimal(str(yes_mid))
            changed = True
        else:
            # Fallback to /midpoint if orderbook is empty
            yes_mid = clob.get_midpoint(yes_token_id)
            if yes_mid is not None:
                match.yes_price = Decimal(str(yes_mid))
                changed = True

        # Get real-time NO mid price from CLOB orderbook (don't derive from YES!)
        # YES and NO have separate order books - prices don't sum to 100%
        no_price = None
        if no_token_id:
            no_price = clob.get_midpoint(no_token_id)  # Returns None if no liquidity

        if no_price is not None:
            match.no_price = Decimal(str(no_price))
            changed = True
        elif match.yes_price:
            # Fallback: derive from YES (when NO has no liquidity or no token ID)
            match.no_price = Decimal(str(1 - float(match.yes_price)))
            changed = True
        ...
        with get_session() as db:
            ...
            for match in matches:
                try:
                    ...
                    # Fetch BOTH YES and NO prices - they have separate order books
                    price_changed = _update_match_from_clob(
                        match, clob, market.yes_token_id, market.no_token_id, now
                    )
