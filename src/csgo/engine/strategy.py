...
class Tick:
    ...
    def yes_price(self) -> Optional[float]:
        """
        Get YES token price (mid-price or last trade).

        Converts from NO price if this tick is for the NO token.
        """
        if self.token_type == "YES":
            return self.mid_price or self.price
        elif self.token_type == "NO":
            # Invert: NO price = 1 - YES price
    ...
    def no_price(self) -> Optional[float]:
        """
        Get NO token price (mid-price or last trade).

        Converts from YES price if this tick is for the YES token.
        """
        if self.token_type == "NO":
            return self.mid_price or self.price
        elif self.token_type == "YES":
            raw = self.mid_price or self.price
            return 1 - raw if raw is not None else None
        return None
