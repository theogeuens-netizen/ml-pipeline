...
class CSGOExecutor:
    ...
    def _calculate_fill_price(
        ...
        Add minimal size impact on top (0.1% per $100).
        ...
        """
        DEFAULT_SPREAD = 0.01  # 1% default spread assumption when no data
        effective_spread = spread if spread else DEFAULT_SPREAD
        ...
        if side == "BUY":
            ...
            if best_ask and best_ask > 0:
                ...
                effective_ask = base_price + (spread / 2)
                effective_bid = base_price - (spread / 2)
            else:
                fill_price = base_price * (1 + DEFAULT_SPREAD / 2)
                effective_spread = DEFAULT_SPREAD
                effective_ask = base_price * (1 + DEFAULT_SPREAD / 2)
                effective_bid = base_price * (1 - DEFAULT_SPREAD / 2)
        else:
            # Cross to bid side
