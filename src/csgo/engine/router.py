...
class CSGOTickRouter:
    ...
    def _enrich_tick(self, tick: Tick) -> Tick:
        ...
        if match:
            ...
            # CRITICAL: Use match price instead of noisy tick price
            # Tick prices are raw trades at various orderbook levels, not actual market price
            if match.get("yes_price"):
                # Set mid_price based on token type so yes_price property uses it
                if tick.token_type == "YES":
                    updates["mid_price"] = match["yes_price"]
                else:
                    updates["mid_price"] = match.get("no_price", 1 - match["yes_price"])
            ...
            if updates:
                ...
                tick_dict = {
                    ...
                    "spread": updates.get("spread", tick.spread),  # Use match spread if available
                    "mid_price": updates.get("mid_price", tick.mid_price),  # Use match price if available
